...
// Função para desenhar um caractere levando em consideração alterações na escala

void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y, uint8_t scale)
{
    uint16_t index = 0;

    if (c >= '0' && c <= '9') {
        index = (c - '0' + 1) * 8; // Números começam no índice 1
    } else if (c >= 'A' && c <= 'Z') {
        index = (c - 'A' + 11) * 8; // Letras maiúsculas começam no índice 11
    } else if (c >= 'a' && c <= 'z') {
        index = (c - 'a' + 37) * 8; // Letras minúsculas começam no índice 37
    } else {
        return; // Ignorar caracteres não suportados
    }

    for (uint8_t i = 0; i < 8; ++i)  // Linhas do caractere original
    {
        uint8_t line = font[index + i];  // Obtém os bits da linha do caractere
        for (uint8_t j = 0; j < 8; ++j)  // Colunas do caractere original
        {
            if (line & (1 << j))  // Se o pixel estiver "aceso" na fonte original
            {
                // Desenha um bloco "scale x scale" no display
                for (uint8_t dx = 0; dx < scale; ++dx)
                {
                    for (uint8_t dy = 0; dy < scale; ++dy)
                    {
                        ssd1306_pixel(ssd, x + i * scale + dx, y + j * scale + dy, true);
                    }
                }
            }
        }
